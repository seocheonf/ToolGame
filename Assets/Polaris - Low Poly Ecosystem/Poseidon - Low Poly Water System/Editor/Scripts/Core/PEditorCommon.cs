...
{
    public static class PEditorCommon
    {
        ...
        public static GUIStyle BoldHeader
        {
            get
            {
                if (boldHeader == null)
                {
                    boldHeader = new GUIStyle(EditorStyles.label);
        ...
        public static int SelectionGrid(PSelectionGridArgs args)
        {
            EditorGUILayout.GetControlRect(GUILayout.Height(1));
            EditorGUILayout.BeginHorizontal();
            EditorGUILayout.GetControlRect(GUILayout.Width(EditorGUI.indentLevel * PEditorCommon.indentSpace));
            int selectedIndex = args.selectedIndex;
            if (args.categorizeFunction != null)
            {
                selectedIndex = SelectionGridCategorized(args);
            }
            else
            {
                selectedIndex = SelectionGridUncategorized(args);
            }
            EditorGUILayout.EndHorizontal();
            EditorGUILayout.GetControlRect(GUILayout.Height(1));
        ...
        private static int SelectionGridCategorized(PSelectionGridArgs args)
        {
            ...
            for (int i = 1; i <= items.Count; ++i)
            {
                ...
                if (currentCategory == null || !currentCategory.Equals(lastCategory))
                {
                    ...
                    PSelectionGridArgs tmpArgs = new PSelectionGridArgs();
                    ...
                    selectedIndex = SelectionGridInternal(tmpArgs, categoryStartIndex);
        ...
        private static int SelectionGridUncategorized(PSelectionGridArgs args)
        {
            return SelectionGridInternal(args, 0);
        }

        private static int SelectionGridInternal(PSelectionGridArgs args, int startIndex = 0)
        {
            EditorGUILayout.BeginVertical();
            ...
            for (int i = 0; i < itemPerRow * numberOfRow; ++i)
            {
                ...
                if (i < args.collection.Count)
                {
                    ...
                    if (args.selectedIndex == currentItemIndex)
                    {
                        ...
                        EditorGUI.DrawRect(highlightRect, PEditorCommon.selectedItemColor);
                    }
        ...
        public static T ObjectSelectorDragDrop<T>(Rect r, string message, string filter = "") where T : class
        {
                ...
                e.type == EventType.Repaint))
            {
                    ...
                    draggedObject[0] is T)
                {
                    ...
                    EditorGUI.DrawRect(r, PUtilities.GetColor(PEditorCommon.selectedItemColor, 0.5f));
                    PEditorCommon.DrawOutlineBox(r, PEditorCommon.selectedItemColor);
                }
            }
        ...
        public static Texture InlineTextureField(string label, Texture value, int indentScope = 0)
        {
            EditorGUILayout.BeginHorizontal();
            EditorGUILayout.PrefixLabel(label);
            using (EditorGUI.IndentLevelScope level = new EditorGUI.IndentLevelScope(indentScope))
            {
                value = EditorGUILayout.ObjectField(value, typeof(Texture), false) as Texture;
            }
            EditorGUILayout.EndHorizontal();
            return value;
        }
        ...
        public static string GetClassDisplayName(Type t)
        {
            ...
            object[] alternativeClassNameAttributes = t.GetCustomAttributes(typeof(PDisplayName), false);
            if (alternativeClassNameAttributes != null && alternativeClassNameAttributes.Length > 0)
            {
                PDisplayName att = alternativeClassNameAttributes[0] as PDisplayName;
        ...
        {
            string prefKey = PEditorCommon.GetProjectRelatedEditorPrefsKey("foldout", prefkeys);
        ...
        public static Rect BeginGroup()
        {
            ...
            GUI.Box(PEditorCommon.boxOffset.Add(r), "");
            return r;
        }
        ...
        {
            string prefKey = PEditorCommon.GetProjectRelatedEditorPrefsKey("foldout", id);
            SessionState.SetBool(prefKey, true);
        }

        public static bool IsFoldoutExpanded(string id)
        {
            string prefKey = PEditorCommon.GetProjectRelatedEditorPrefsKey("foldout", id);
        ...
        {
            EditorGUI.DrawRect(r, PEditorCommon.assetPreviewGrey);
            ...
            else
            {
                ...
                {
                    EditorGUI.LabelField(r, "Loading preview...", PEditorCommon.CenteredWhiteLabel);
                }
                else
                {
                    EditorGUI.LabelField(r, "Preview not available!", PEditorCommon.CenteredWhiteLabel);
                }
            }
        }
