...
namespace Pinwheel.Poseidon
{
    ...
    public static class PEditorMenus
    {
        ...
        [MenuItem("GameObject/3D Object/Poseidon/Calm Water HQ/Standard")]
        public static PWater CreateCalmWaterHQObject(MenuCommand cmd)
        {
            GameObject g = new GameObject("Calm Water HQ Standard");
            if (cmd != null && cmd.context != null)
            {
                GameObject root = cmd.context as GameObject;
                GameObjectUtility.SetParentAndAlign(g, root);
            ...
            PWater waterComponent = g.AddComponent<PWater>();
            PWaterProfile profile = Resources.Load<PWaterProfile>("OuterAssets/PostProcessProfiles/StandardWaterProfile");
            waterComponent.Profile = profile;
            waterComponent.TileSize = new Vector2(10, 10);

            /*
            PWater waterComponent = g.AddComponent<PWater>();
            PWaterProfile profile = PWaterProfile.CreateInstance<PWaterProfile>();
            string fileName = "WaterProfile_" + PCommon.GetUniqueID();
            string filePath = string.Format("Assets/PostProcessProfiles/{0}.asset", fileName);
            AssetDatabase.CreateAsset(profile, filePath);
        ...
        [MenuItem("GameObject/3D Object/Poseidon/Calm Water HQ/Unique")]
        public static PWater CreateCalmWaterHQObjectUnique(MenuCommand cmd)
        {
            GameObject g = new GameObject("Calm Water HQ Unique");
            if (cmd != null && cmd.context != null)
            {
                GameObject root = cmd.context as GameObject;
                GameObjectUtility.SetParentAndAlign(g, root);
            ...
            PWaterProfile standardProfile = Resources.Load<PWaterProfile>("OuterAssets/PostProcessProfiles/StandardWaterProfile");
            
            PWater waterComponent = g.AddComponent<PWater>();
            PWaterProfile profile = PWaterProfile.CreateInstance<PWaterProfile>();
            string fileName = "WaterProfile_" + PCommon.GetUniqueID();
            string filePath = string.Format("Assets/Resources/OuterAssets/PostProcessProfiles/{0}.asset", fileName);
            AssetDatabase.CreateAsset(profile, filePath);

            profile.CopyFrom(standardProfile);
